...
def _flat_graph_name(graph: str) -> str:
    """Collapse preset-qualified graph names for binary file names."""
...
def graph_binary_path(graph: str, mode: str) -> Path:
    return graph_build_dir(graph, mode) / f"{_flat_graph_name(graph)}_test"


def graph_m5out_dir(graph: str, mode: str, tag: str | None = None) -> Path:
    _check_mode(mode)
    mode_dir = tag or mode
    return M5OUT_ROOT / "graphs" / graph / mode_dir / "default"


def graph_roi_stats_path(graph: str, mode: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, mode, tag) / "roi-stats.txt"


def graph_run_log_path(graph: str, mode: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, mode, tag) / "run.log"


def graph_compare_path(graph: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, "spm", tag) / "compare_vs_cache.txt"


def graph_spm_stats_path(graph: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, "spm", tag) / "spm_stats.txt"


def graph_report_path(graph: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, "spm", tag) / "graph_report.txt"


def graph_fused_build_dir(graph: str) -> Path:
    return BUILD_ROOT / "graphs" / graph / "fused"


def graph_fused_binary_path(graph: str) -> Path:
    return graph_fused_build_dir(graph) / f"{_flat_graph_name(graph)}_fused_test"


def graph_fused_m5out_dir(graph: str) -> Path:
    return M5OUT_ROOT / "graphs" / graph / "fused" / "default"


def graph_fused_roi_stats_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "roi-stats.txt"


def graph_fused_run_log_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "run.log"


def graph_fused_compare_cache_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "compare_vs_cache.txt"


def graph_fused_compare_unfused_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "compare_vs_unfused_spm.txt"


def graph_fusion_report_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "fusion_report.txt"


def graph_fusion_json_path(graph: str) -> Path:
    return graph_fused_m5out_dir(graph) / "fusion_report.json"


def graph_eval_json_path(graph: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, "spm", tag) / "graph_eval.json"


def graph_eval_summary_path(graph: str, tag: str | None = None) -> Path:
    return graph_m5out_dir(graph, "spm", tag) / "graph_summary.txt"
...
def main() -> None:
    ...
    p.add_argument(
        ...
        choices=[
            ...
            "graph_spm_stats",
            "graph_report",
            "graph_fused_build_dir",
            "graph_fused_binary",
            "graph_fused_m5out_dir",
            "graph_fused_roi_stats",
            "graph_fused_run_log",
            "graph_fused_compare_cache",
            "graph_fused_compare_unfused",
            "graph_fusion_report",
            "graph_fusion_json",
            "graph_eval_json",
            "graph_eval_summary",
    ...
    if args.what in (
        ...
        fns = {
            ...
            "graph_fused_build_dir": graph_fused_build_dir,
            "graph_fused_binary": graph_fused_binary_path,
            "graph_fused_m5out_dir": graph_fused_m5out_dir,
            "graph_fused_roi_stats": graph_fused_roi_stats_path,
            "graph_fused_run_log": graph_fused_run_log_path,
            "graph_fused_compare_cache": graph_fused_compare_cache_path,
            "graph_fused_compare_unfused": graph_fused_compare_unfused_path,
            "graph_fusion_report": graph_fusion_report_path,
            "graph_fusion_json": graph_fusion_json_path,
