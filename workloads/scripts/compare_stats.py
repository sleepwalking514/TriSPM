...
DRAM_READ_STAT = "system.mem_ctrl.dram.dramBytesRead"
DRAM_WRITE_STAT = "system.mem_ctrl.dram.dramBytesWritten"
...
COMPARE_FIELDS = [
    ...
    ("dram.bytesRead", DRAM_READ_STAT),
    ("dram.bytesWritten", DRAM_WRITE_STAT),
...
def sum_stats(stats: dict[str, str], stat_names: tuple[str, ...]) -> str | None:
    values: list[float] = []
    for stat_name in stat_names:
        value = as_number(stats.get(stat_name))
        if value is None:
            return None
        values.append(value)
    total = sum(values)
...
def compare_rows(spm: dict[str, str], cache: dict[str, str], measure_iters: int) -> list[tuple[str, ...]]:
    ...
    spm_dram_total = sum_stats(spm, (DRAM_READ_STAT, DRAM_WRITE_STAT))
    cache_dram_total = sum_stats(cache, (DRAM_READ_STAT, DRAM_WRITE_STAT))
    rows.append((
        "dram.bytesTotal",
        fmt(spm_dram_total),
        fmt(cache_dram_total),
        fmt_delta(spm_dram_total, cache_dram_total),
    ))
...
def render_compare(
    ...
    return render_table(
        compare_rows(spm, cache, measure_iters),
        ("stat", spm_label, cache_label, "delta"),
    )
...
def main() -> None:
    ...
    parser.add_argument("--spm-label", default="spm",
                        help="column label for the --spm stats")
    parser.add_argument("--cache-label", default="cache",
                        help="column label for the --cache stats")
    ...
    compare_text = render_compare(
        spm,
        cache,
        args.measure_iters,
        spm_label=args.spm_label,
