...
#endif
#ifndef GRAPH_FLASH_CAUSAL
#define GRAPH_FLASH_CAUSAL 0
#endif
...
int main(void)
{
    printf("graph attention_smoke: SEQ=%d D_MODEL=%d HEAD_DIM=%d FFN_DIM=%d BLOCK=%d flash=%d check=%d flush=%d\n",
           GRAPH_SEQ, GRAPH_D_MODEL, GRAPH_HEAD_DIM, GRAPH_FFN_DIM, GRAPH_BLOCK,
           GRAPH_USE_FLASH_ATTENTION, GRAPH_CHECK_RESULT, GRAPH_FLUSH_BEFORE_ROI);
    const float flash_sm_scale = 1.0f / sqrtf((float)GRAPH_HEAD_DIM);
    ...
    float *scores = NULL;
    float *probs = NULL;
    float *attn = (float *)flash_attention_alloc(3, qkv_bytes);
#else
    ...
    if (!x_shadow || !gamma_shadow || !beta_shadow || !wq_shadow ||
        ...
        (!GRAPH_USE_FLASH_ATTENTION && (!k_t || !scores || !probs)) ||
        !attn || !wo || !attn_proj || !residual ||
...
#if GRAPH_USE_FLASH_ATTENTION
    flash_attention_launch(FLASH_GRID_X, 1, 1, q, k, v, attn, flash_sm_scale);
#else
    ...
    if (GRAPH_CHECK_RESULT) {
...
#if GRAPH_USE_FLASH_ATTENTION
        reference_flash_attention(q_ref, k_ref, v_ref, attn_ref, flash_sm_scale);
#else
        ...
        report_tensor_error("q", q, q_ref, QKV_ELEMS);
        report_tensor_error("k", k, k_ref, QKV_ELEMS);
        report_tensor_error("v", v, v_ref, QKV_ELEMS);
        report_tensor_error("attn", attn, attn_ref, QKV_ELEMS);
        report_tensor_error("attn_proj", attn_proj, attn_proj_ref, MODEL_ELEMS);
