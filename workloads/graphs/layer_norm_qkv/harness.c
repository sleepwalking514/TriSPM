...
int main(void)
{
    ...
    float *wq = (float *)q_proj_alloc(1, weight_bytes);
    float *wk = (float *)k_proj_alloc(1, weight_bytes);
    float *wv = (float *)v_proj_alloc(1, weight_bytes);
    float *q = (float *)q_proj_alloc(2, proj_bytes);
    float *k = (float *)k_proj_alloc(2, proj_bytes);
    float *v = (float *)v_proj_alloc(2, proj_bytes);
    ...
    q_proj_launch(MATMUL_GRID_X, 1, 1, ln_out, wq, q);
    k_proj_launch(MATMUL_GRID_X, 1, 1, ln_out, wk, k);
    v_proj_launch(MATMUL_GRID_X, 1, 1, ln_out, wv, v);
