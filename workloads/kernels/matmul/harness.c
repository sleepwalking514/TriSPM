...
int main(void)
{
    ...
    /* Cacheable shadows for inputs.  The measured input buffers are allocated
     * by the launcher, while init/reference stay outside the measured ROI. */
    ...
    /* Publish inputs into the launcher-chosen ordinary DRAM buffers. */
    publish_input(a, a_shadow, a_bytes);
    publish_input(b, b_shadow, b_bytes);

    /* Zero output in the measured output buffer. */
    memset(c, 0, c_bytes);
    ...
    if (matmul_check_result) {
        ...
        /* Reference matmul on private shadow buffers after the measured ROI. */
        for (int i = 0; i < M; i++)
            for (int j = 0; j < N; j++) {
                float sum = 0.0f;
