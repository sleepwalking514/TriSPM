...
N and BLOCK_SIZE_N are constexprs, with BLOCK_SIZE_N kept at 8 (one
...
def layer_norm(x_ptr, gamma_ptr, beta_ptr, out_ptr,
    ...
    x_mean_ptr = tl.make_block_ptr(
        ...
        offsets=(row * N,), block_shape=(BLOCK_SIZE_N,), order=(0,))
    ...
    for off in range(0, N, BLOCK_SIZE_N):
        ...
        x_mean_ptr = tl.advance(x_mean_ptr, (BLOCK_SIZE_N,))
    mean = _sum / N
    ...
    for off in range(0, N, BLOCK_SIZE_N):
        ...
        x_var_ptr = tl.advance(x_var_ptr, (BLOCK_SIZE_N,))
    var = _var / N
...
layer_norm[(GRID_X,)](x, gamma, beta, out, M, N, BLOCK_SIZE_N)
