...
N and BLOCK_N are constexprs. BLOCK_N defaults to 8 in experiment.toml
(one RISC-V vector register, LMUL=1 at VLEN=256), but is exported so it can
be tuned. The default avoids two gem5 RVV bugs:
...
BLOCK_N = env_int("BLOCK_N")  # process N in chunks
GRID_X = M              # one program per row; SPM pass may group rows later

if BLOCK_N <= 0:
    raise ValueError("layer_norm requires positive BLOCK_N")
if N % BLOCK_N != 0:
    raise ValueError("layer_norm workload requires N to be divisible by BLOCK_N")
