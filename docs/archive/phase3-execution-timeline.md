...
overview: `matmul-spm-lowering-closure.md` 的 matmul P3 cold-start 主线已收敛并已看到 SPM crossover，但最终 headline 需要 SPM/cache blocking sweep 后再定；Tier sidecar、L2-warming、评测工具化、reduction 2-D 地址修复、single-load reduction 双缓冲、multi-load reduction matcher、bail-out cleanup/verification、Phase 4 前 transformer-facing workload smoke coverage、graph-level conservative placement build/verify MVP、explicit promotion D1 evidence/reporting、D2 row-resident reduction opt-in prototype 和 D3 conservative profitability/rejection gate 均已完成。LayerNorm/reduction SPM 现在默认关闭，仅保留 opt-in correctness/coverage/evidence 路径。当前主线在 Phase 3.5 P2f/P3：Softmax kernel 回到 canonical，cache baseline 使用 stock Triton CPU scheduling，row-block/group DMA 作为 SPM-only compiler transform 由 `SPM_ROW_BLOCK`/`SPM_ROW_GROUP_BLOCKS` 调优；attention/fusion 可执行原型、producer-consumer promotion 和 Phase 6 评测扩展排在 reduction closure 之后。
tasks:
  ...
  - id: explicit-promotion-d2
    ...
    status: completed
    note: 2026-05-03 D2 完成。新增 `TRITON_ENABLE_SPM_ROW_RESIDENT_REDUCTIONS=1` / `enable-row-resident-reductions` 和 `TRITON_SPM_ROW_RESIDENT_MAX_BYTES` / `row-resident-max-bytes`；backend 在 row-resident flag 打开时不把旧 streaming reduction tier sidecar 混进来。验证覆盖：默认 `make verify-layer_norm` 仍无 SPM marker 且 tier JSON `{}`；旧 `TRITON_ENABLE_SPM_REDUCTIONS=1` 仍命中 streaming coverage 且 tier JSON non-empty；row-resident opt-in 会生成 SPM marker、tier JSON `{}`、promotion sidecar accepted source `LayerNorm x row`；两个 flag 同时打开时仍走 row-resident evidence 且 tier JSON `{}`。性能仍不达标：32x64 为 10,279 vs 6,138 cycles（+67.5%），512x1024 为 1,245,422 vs 1,012,922 cycles（+23.0%）。D1 sidecar 在 D2 只记录 accepted/rejected evidence，不驱动 planner、调度、`windowK`、buffer layout 或 profitability；D3 已补上 conservative profitability/rejection model。
  - id: explicit-promotion-d3
    priority: high
    content: 为 explicit SPM promotion 加一个 conservative static profitability/rejection gate：记录 descriptor/MMIO/wait/fence/bytes/uses evidence，接受现有 fused matmul promotion evidence，拒绝当前 streaming/row-resident LayerNorm reduction，且不把 D1 sidecar 升级成 planner 接口。
    status: completed
    note: 2026-05-03 D3 完成。新增 `TRITON_ENABLE_SPM_PROMOTION_PROFITABILITY=1` / `enable-promotion-profitability`；profitability evidence 写入 D1 debug sidecar 的 `profitability` 字段，但 compiler 不读回该 sidecar，不驱动 `windowK`、placement、buffer layout 或调度。lit 覆盖 fused matmul accept evidence、streaming reduction rejection、row-resident rejection。workload 验证：默认 `make verify-layer_norm` 仍 cache path；D2 row-resident opt-in 在 D3 gate 关闭时仍可 promotion；D3 gate 打开时 row-resident LayerNorm 被 `insufficient_row_work` 拒绝且 LLIR/tier JSON 保持 clean；旧 streaming reduction 被 `streaming_reduction_no_residency` 拒绝且 tier JSON `{}`。当前 single-kernel D1-D3 promotion/rejection groundwork 可视为收敛，但 2026-05-04 的 P2c Softmax row-block DMA win 需要先回填 D3/P3 profitability refit，之后再转向 Gate B attention/fusion 和 producer-consumer promotion。
  - id: p3-profile-overlap
    content: 画清 matmul prefetch enqueue、current buffer 读取、下一轮 dma_wait 成功之间的时间线，确认是否存在真实 overlap。
    status: completed
    note: Stage 1 完成。结论见 `matmul-spm-lowering-closure.md` §P3.1：DMA 已 62.3% 重叠（avgWaitStall 60.68/avgLatency 160.92），瓶颈不在 prefetch 完成，而是 MMIO descriptor stores 引发的 back-end 阻塞（占 cycle gap 68.8%）。
  - id: fair-baseline-flush
    content: harness 在 measured region 之前 flush cache（Zicbom 或 scrub buffer）、加速 SPM 端 host check（DMA copy 到 cacheable）；SPM/cache 两侧 cold-cache 重测，更新 matmul-spm-lowering-closure.md §P3.0/§P3.1。
    status: completed
    note: Stage 2.5 完成。harness 加 publish_input + flush_caches；cold-cache fair baseline gap 从 +75.9% 降到 +6.7%。结果记入 `matmul-spm-lowering-closure.md` §P3.0。后续 Stage 2.6 已补 sweep 数据；cold-start 是 matmul P3 的关键口径，steady-state warm-cache 仅作辅助参考。
  ...
  - id: steady-state-and-size-sweep
    content: harness 加 N-iter steady-state 测量模式 + 128×128/256×256 size sweep；补 `matmul-spm-lowering-closure.md` §P3.5 的 warm-cache 与大尺寸数据。
    status: completed
...
> **Status: ARCHIVED RESULT — 2026-05-05**
> This timeline records the completed Phase 3 sequence. Current work now lives
> in [`../plans/phase3.5-single-kernel-convergence.md`](../plans/phase3.5-single-kernel-convergence.md),
...
- `matmul-spm-lowering-closure.md` 的 P0-P2 已完成：matmul 汇编形态已修复，`vfmacc.vf=256`、`vrgather=0`、spill/reload 与 cache baseline 对齐。
...
- Explicit promotion D3 已完成 conservative static gate：
  `TRITON_ENABLE_SPM_PROMOTION_PROFITABILITY=1` 会记录 descriptor/MMIO/wait/
  fence/bytes/uses evidence，接受现有 fused matmul promotion evidence，并拒绝
  当前 streaming/row-resident LayerNorm reduction；rejected reduction 不会留下
  SPM LLIR marker 或非空 tier JSON。D1 sidecar 仍是 debug/evidence，不是 planner
  或调度接口。
...
11. Done: implement D3 conservative promotion profitability/rejection logic for current single-kernel coverage.
12. Current high priority before a full transformer driver: finish Phase 3.5 P2f/P3 reduction closure.  Softmax now uses a canonical shared kernel; the stock cache baseline stays unmodified, while the SPM path owns row-block/group DMA tuning (`SPM_ROW_BLOCK=2` is stable so far, `SPM_ROW_GROUP_BLOCKS` remains shape-sensitive).  Refit D3 profitability so the compiler can accept that SPM-only transform while continuing to reject LayerNorm/small-row cases.
13. Current optimization line: continue `spm-dma-reuse.md` fused microM-aware scheduler tuning and larger-run validation.
14. Current blocker to record and later investigate: opt-in streaming and row-resident reduction SPM performance are both behind cache even though functional coverage passes; D3 now rejects them by policy.
15. Later: enter `three-tier-placement.md` §6.1 for Tier 1 resident SPM and §6.3 for workload-driven scalar-reuse rule expansion.
...
- **交付物**：在 `matmul-spm-lowering-closure.md` 新增 "P3 Timeline" 小节，给出 cycle 预算表 + 单次 iteration 时序示意 + Stage 3 候选干预的优先级排序。
...
- **交付物**：harness 改动（`workloads/matmul/...` + 共享 helper） + 一份 fair-baseline 数据表（cold-cache SPM vs cold-cache cache）写进 `matmul-spm-lowering-closure.md` §P3.1 之后的新小节 `P3.0 Fair baseline`，并在 §P3.1 顶部加 caveat 说明原数据未做 flush。
- **验证**：
...
- **交付物**：`matmul-spm-lowering-closure.md` §P3.5 补 steady-state + 大尺寸数据；论文头数应从大尺寸里挑。
...
- **背景**：`transformGemmLoop` 已是双缓冲 + prologue prefetch（matmul-spm-lowering-closure.md §P3.1 实测 DMA 延迟 62.3% 被 overlap 隐藏）。当时 `transformReductionLoop` 仍是 *单缓冲* prefetch：每轮 body 先 `vector.transfer_read` 当前 chunk → 再 `dma_enqueue_2d` 下一 chunk → `dma_wait`，CPU 必须等 DMA 完成才能进入下一轮。这意味着 layer_norm / softmax / 未来所有 reduction kernel 的 DMA 延迟全部串行暴露在关键路径，是 reduction 路径性能基线偏弱的结构性原因。
...
- `phase3.md` 现在是同步后的 Phase 3 状态页；如果它与本计划冲突，应优先重新核对 `matmul-spm-lowering-closure.md`、`../plans/three-tier-placement.md`、`../evidence/l2_warming.md` 和实际生成物。

## 关键参考文件
- [`phase3.md`](phase3.md): archived Phase 3 状态页。
- [`matmul-spm-lowering-closure.md`](matmul-spm-lowering-closure.md): matmul P3 closure and measured result log.
- [`../plans/three-tier-placement.md`](../plans/three-tier-placement.md): 三层 placement 的设计与 MVP 里程碑。
