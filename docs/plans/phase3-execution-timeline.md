...
12. Current high priority before a full transformer driver: finish Phase 3.5 P2c/P3 reduction closure.  The tuned Softmax row-block DMA preset (`ROW_BLOCK=2`, `ROW_GROUP_BLOCKS=8`) now beats cache; refit D3 profitability so the compiler can accept that case while continuing to reject LayerNorm/small-row cases.
